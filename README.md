...
- 学历写法推荐 “统招本科/全日制本科”，避免出现“2年/两年”字样。
...
export interface Resume {
  basics: {
    ...
    github?: string;
    website?: string;
    contact?: { email?: string; phone?: string; wechat?: string; city?: string };
  };
  ...
  education: { school: string; major: string; degree: string; graduation?: string; nature?: string }[];
}
```
...
### 设计守则
- 单栏、黑白灰，不使用头像、图标、进度条、时间轴。
- 列表间距 6-8px，模块间距 16-24px，正文默认 11-12px，行距默认 1.4（可调 1.2-1.6）。
- 学历描述遵循“统招本科/全日制本科”安全写法，不暴露学制时长。
