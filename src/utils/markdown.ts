...
const parseExperienceHeading = (heading: Heading): Experience => {
  ...
  const [position = "", company = "", duration = ""] = title
    .split(/[|｜]/)
    .map((s) => s.trim());
...
const extractNestedListItems = (item: ListItem): string[] => {
  const nestedList = item.children.find((child) => child.type === "list") as
    | List
    | undefined;
  if (!nestedList) return [];
  return nestedList.children
    .map((child) => listItemText(child as ListItem))
    .filter(Boolean);
};
...
export const parseMarkdown = (markdown: string): Resume => {
  ...
  const normalizeContributionsIndent = (raw: string): string => {
    ...
    for (let i = 0; i < lns.length; i += 1) {
      ...
      for (let j = i + 1; j < lns.length; j += 1) {
        ...
        if (
          next.startsWith(baseIndent) &&
          next.match(new RegExp(`^${baseIndent}-\\s+`))
        )
          break;
  ...
  tree.children.forEach((node) => {
    if (node.type === "heading") {
      ...
      if (heading.depth === 1) {
        resume.basics.name = heading.children
          .map(normalizeText)
          .join(" ")
          .trim();
    ...
    if (node.type === "list" && currentSection === SECTION_TYPE_EDU) {
      ...
      const list = node as List;
      list.children.forEach((item) => {
        const parts = listItemText(item as ListItem)
          .split(/[|｜]/)
          .map((s) => s.trim());
        const [school = "", major = "", degree = "", graduationRaw = ""] =
          parts;
        ...
        for (let idx = 0; idx < list.children.length; idx += 1) {
          ...
          if (isContributions) {
            ...
            const contentNodes = children.filter((child) => {
              ...
              const pText = normalizeText(child as Paragraph)
                .toLowerCase()
                .replace(/[*_`]/g, "")
                .trim();
              // remove both `contributions:` and `contributions: xxx`
              return !(
                pText === `${KEY_CONTRIBUTIONS}:` ||
                pText.startsWith(`${KEY_CONTRIBUTIONS}:`)
              );
            });
            if (contentNodes.length)
              parts.push(stringifyMdastNodes(contentNodes));
            ...
            for (let j = idx + 1; j < list.children.length; j += 1) {
              ...
              if (nextNestedList)
                parts.push(stringifyMdastNodes([nextNestedList]));
  ...
  if (Object.keys(frontMatter).length) {
    ...
    resume.basics = {
      ...resume.basics,
      ...fmBasics,
      contact: { ...resume.basics.contact, ...fmBasics.contact },
    };
...
export const resumeToMarkdown = (resume: Resume): string => {
  ...
  if (
    ...
    if (resume.basics.contact?.email)
      fm.push(`email: ${resume.basics.contact.email}`);
    if (resume.basics.contact?.wechat)
    ...
    if (resume.basics.school)
      fm.push(`education_school: ${resume.basics.school}`);
    if (resume.basics.major) fm.push(`education_major: ${resume.basics.major}`);
    if (resume.basics.degree)
      fm.push(`education_degree: ${resume.basics.degree}`);
    if (resume.basics.graduation)
      fm.push(`education_gradYear: ${resume.basics.graduation}`);
    fm.push("---", "");
  }
